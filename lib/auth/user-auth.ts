...
import { users, teams, teamMembers, ActivityType, NewUser, invitations, Invitation, NewTeam } from "@/lib/db/schema";
...
export function logErrorResult<T, E extends AuthError>(
    ...
    if (!result.ok) {
        console.error(
            `[${context}] error!:`, 
            { errorCode: result.error.error, details: result.error.message }
        );
        return false;
    }
    return result.value;
...
export async function authenticateUser(user: User, password: string): Promise<void> {
  if(user.name){
...
export async function findInvitation(inviteId: string, email: string): Promise<Result<Invitation>> {
...
export async function acceptInvitation(invitationId: number): Promise<Result<void>> {
    try {
      await db
        .update(invitations)
        .set({ status: 'accepted' })
...
export async function signInUserInterface(email: string, password: string): Promise<Result<User>> {
    //유저 팀 찾기
